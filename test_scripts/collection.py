...
#     print(os_rain_state)
    ...
    glider_ship_merged = glider_ship_merged.sort_values(['TYPE','DETAILED TYPE'])
    ...
    glider_ship_merged.sort_values(by='TYPE', inplace=True)
    glider_ship_merged.reset_index(drop=True, inplace=True)
    glider_ship_merged.loc['Total'] = glider_ship_merged.sum()
    glider_ship_merged.loc[glider_ship_merged.index[-1], 'TYPE'] = 'Total'
    ...
    glider_ship_type = glider_ship_merged.groupby('TYPE').sum()
